...
- [x] **`transformers<5.0.0` vs. the DeBERTa checkpoint, resolved (2026-08-17).** Root cause was two-layered: (1) the checkpoint needs `transformers>=5.x` to load its tokenizer (confirmed, see prior note -- this repo's `requirements.txt` now reads `transformers>=5.0.0,<6.0.0`); (2) `transformers>=5.x` itself needs `torch>=2.5`, which this Mac's `.venv` couldn't install because **that venv's Python was an x86_64 (Intel) build running under Rosetta 2** on this Apple Silicon machine -- PyTorch dropped Intel-macOS wheels after 2.2.x, so that Python could never reach `torch>=2.5` regardless of the `transformers` pin. Fixed by rebuilding `.venv` from a native arm64 Python (`arch -arm64 /usr/local/bin/python3.10 -m venv`) -- verified in isolation first (new venv in a scratch dir, all 170 tests passing, DeBERTa loading and predicting for real) before swapping it in for the real `.venv` (old one kept renamed as a backup until the swap was confirmed safe, then deleted). Confirmed no impact on the pre-existing sarcasm-interpretation pipeline's one `transformers` usage (`src/evaluation/evaluate_with_nli.py`, standard `AutoTokenizer`/`AutoModelForSequenceClassification.from_pretrained` on a same-family DeBERTa-v2 model -- low risk, and the full 150-test main suite plus 20-test web suite both pass under the new environment). `deberta` now genuinely reports `AVAILABLE` on this local Mac.
- [x] **The OpenRouter live-demo decision -- resolved by the user (2026-08-17): "live demo via a clearly-labeled substitute model" (option a).** Model of choice: `openai/gpt-oss-20b:free` (already proven through this repo's OpenRouter integration as the judge model, `config/models.py`'s `JUDGE_MODELS["openrouter_llm_judge"]` -- `qwen/qwen3-4b:free`, the originally-planned substitute, is no longer served by OpenRouter's free tier as of this session, confirmed via a live `/models` API call). A working key was provided and verified live (`.env`'s `OPENROUTER_API_KEY`, gitignored, never committed). **Implementation was started (schemas.py's `note` field on `MethodInfo`/`MethodPrediction`/`PredictResponse`, `base.py`'s `BaseAdapter.backend_note`, `qwen_adapter.py`'s OpenRouter fallback in `_lazy_load`) but reverted (`git checkout --`) when the user explicitly decided to abandon the web app entirely mid-implementation** -- reverting, not leaving a half-wired feature, was the deliberate choice once that call was made. **If web app work resumes later**, this design (a `backend_note`/`note` field disclosing "live demo via a substitute model, not the frozen benchmark" surfaced through the API/frontend) is still the agreed approach -- pick it back up rather than re-deciding from scratch, and remember `dspy_adapter.py` needs the equivalent fallback too (not yet started when the pause happened).
- [x] **Web app work paused/abandoned by explicit user request (2026-08-17)**, mid-way through the OpenRouter fallback implementation above. State left clean and fully tested (170/170, both `tests/` and `web/backend/tests/`) -- not half-broken, just stopped. What *is* done and stays done: `qwen_adapter.py`'s registry-driven few-shot config, `dspy_adapter.py`'s frozen compiled-program loading, `sentencepiece` + `transformers>=5.0.0` fixes (both benefit the base repo/reproducibility regardless of the web app's fate, not web-app-only changes). What's *not* done and won't be unless explicitly picked back up: the OpenRouter fallback (reverted, see above), any frontend changes, `web/frontend/` verification against the current backend behavior.
